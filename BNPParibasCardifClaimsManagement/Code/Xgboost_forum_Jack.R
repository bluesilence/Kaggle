...
data.train <- fread("data/raw/train.csv")
data.test <- fread("data/raw/test.csv")
...
# Enumerates the possible pair combinations of character features
cmb <- combinations(n = length(char.vars), r = 2, v = char.vars)
# Generate combined character features
for(i in 1:nrow(cmb)) {
...
# Enumerates the possible pair combinations of character features except v22
...
### WARNING: Causes R Studio crush!
# Enumerates the possible r-dimension combinations of character features except v22
...
for(var in colnames(x.test)) {
  if(is.character(x.test[[var]])) {
    # Get target mean group by var
    target.mean <- x.train[, list(pr = mean(target)), by = eval(var)]
    # Replace var with the target mean of the value of var in each row
    x.test[[var]] <- target.mean$pr[match(x.test[[var]], target.mean[[var]])]
    temp <- rep(NA, nrow(x.train))
    
    # 4-folds CV using 3/4 rows to generate target mean and replace var
    for(i in 1:4) {
...
for(var in colnames(x.test)) {
  if(is.character(x.test[[var]])) {
    x.train[, list(pr = mean(target)), by = eval(var)]
  }
}
...
write.csv(predict, paste0("data/final/Submission_xgboost_forum_Jack.csv"), row.names=FALSE)
